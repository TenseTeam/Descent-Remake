        ...
        private IEnumerator CalculateRotation()
        {
            while (true)
            {
                Vector3 rot = new Vector3
                    (
                    Inputs.Aircraft.Pitch.ReadValue<float>() * (float)pitchAxis,
                    Inputs.Aircraft.Yaw.ReadValue<float>() * (float)yawAxis,
                    Inputs.Aircraft.Roll.ReadValue<float>() * (float)rollAxis
                    );
